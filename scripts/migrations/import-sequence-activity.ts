/**
 * Import SequenceActivity Collection from JSON
 *
 * Purpose: Import SequenceActivity documents from JSON export file into MongoDB
 *
 * Usage:
 *   ts-node scripts/migrations/import-sequence-activity.ts
 *
...
// Configuration
const COLLECTION_NAME = 'SequenceActivity'
const JSON_FILE_PATH = path.join(
  ...
  'v2_UvuyoYogaDb.SequenceActivity.json'
)
...
interface SequenceActivityDocument {
  _id: { $oid: string }
  userId: { $oid: string }
  sequenceId: string
  sequenceName: string
 ...
 */
function transformDocument(doc: SequenceActivityDocument): any {
  return {
    _id: doc._id.$oid,
    userId: doc.userId.$oid,
    sequenceId: doc.sequenceId,
    sequenceName: doc.sequenceName,
...
async function importSequenceActivity() {
  console.log('='.repeat(60))
  console.log('SequenceActivity Collection Import')
  console.log('='.repeat(60))
  console.log()
  ...
  let jsonData: SequenceActivityDocument[]
  try {
...
// Run the import
importSequenceActivity().catch(console.error)
