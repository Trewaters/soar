  ...
  if (sequences.length === 0) {
    return (
      <Box sx={{ textAlign: 'center', py: 4 }}>
        ...
        <Button
          variant="contained"
          onClick={() => router.push('/sequences/createSequence')}
        >
...
function SequenceListItem({
  ...
  return (
    <Paper
      ...
      onClick={() =>
        router.push(`/sequences/practiceSequences?id=${sequence.id}`)
      }
    >
      {/* Sequence info */}
      <Box sx={{ flexGrow: 1, mr: 2 }}>
      ...
      >
        <IconButton
          ...
          onClick={(e) => {
            e.stopPropagation()
            router.push(
              `/sequences/practiceSequences?id=${sequence.id}&edit=true`
            )
          }}
          sx={{ color: 'text.secondary' }}
        >
