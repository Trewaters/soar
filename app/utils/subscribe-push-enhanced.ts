...
// Runtime site hostname derivation (use require to avoid build-time transforms)
let SITE_URL_RUNTIME = ''
try {
  // eslint-disable-next-line @typescript-eslint/no-var-requires
  SITE_URL_RUNTIME = require('../config/site').SITE_URL
} catch (err) {
  SITE_URL_RUNTIME = ''
}

const SITE_HOSTNAME_RUNTIME = (() => {
  try {
    return new URL(SITE_URL_RUNTIME).hostname
  } catch {
    return SITE_URL_RUNTIME.replace(/^https?:\/\//, '')
  }
})()

const CANONICAL_HOSTNAME_RUNTIME = SITE_HOSTNAME_RUNTIME.replace(/^www\./, '')
const ALLOWED_HOSTNAMES_RUNTIME = [
  CANONICAL_HOSTNAME_RUNTIME,
  `www.${CANONICAL_HOSTNAME_RUNTIME}`,
]
...
function analyzePushError(error: any): {
  ...
  // Derive site hostname from central config to allow domain changes in one place
  // Importing here avoids top-level import side-effects in some runtimes
  let SITE_URL = ''
  try {
    // eslint-disable-next-line @typescript-eslint/no-var-requires
    SITE_URL = require('../config/site').SITE_URL
  } catch (err) {
    SITE_URL = ''
  }
  ...
  const allowedHostnames = [canonicalHostname, `www.${canonicalHostname}`]
  ...
  if (
    ...
    const isProduction = ALLOWED_HOSTNAMES_RUNTIME.includes(
