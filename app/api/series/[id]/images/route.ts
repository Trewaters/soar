...
export async function POST(
  ...
  try {
    ...
    // Add the new image URL to the flow images array
    ...
    console.error('Error uploading flow image:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
...
export async function DELETE(
  ...
  try {
    ...
    // Remove the image URL from the flow images array
    const currentImages = series.images || []
    ...
    console.error('Error deleting flow image:', error)
    return NextResponse.json(
      { error: 'Internal server error' },
      { status: 500 }
    )
