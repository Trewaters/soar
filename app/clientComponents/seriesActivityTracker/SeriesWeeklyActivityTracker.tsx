...
interface SeriesWeeklyActivityTrackerProps {
  seriesId: string
  seriesName: string
...
export default function SeriesWeeklyActivityTracker({
  ...
  useEffect(() => {
    const fetchWeeklyData = async () => {
      ...
      try {
        const data = await getSeriesWeeklyActivity(session.user.id, seriesId)
        setWeeklyData(data)
      } catch (e: any) {
          ...
          <>
            ...
            <Box>
              <Typography variant="body2" color="text.secondary" gutterBottom>
                All Series Practice This Week ({weeklyData.activities.length}{' '}
                sessions):
              </Typography>
              <Stack spacing={1}>
