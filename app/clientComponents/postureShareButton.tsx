...
type CombinedPostureShareButtonProps =
  | PostureShareButtonProps
  | LegacyPostureShareButtonProps
...
function isNewProps(
  props: CombinedPostureShareButtonProps
): props is PostureShareButtonProps {
  return 'content' in props
}

// Helper function to convert legacy props to new content format
function convertLegacyProps(
  props: LegacyPostureShareButtonProps
): ShareableContent | null {
  if (props.postureData) {
    return {
      contentType: 'asana',
      data: props.postureData,
    }
  }
  if (props.seriesData) {
    return {
 ...
 * @param {FullAsanaData} [props.postureData] - Legacy: The data for a single yoga posture (deprecated).
 * @param {FlowSeriesData} [props.seriesData] - Legacy: The data for a series of yoga postures (deprecated).
 ...
 * />
 */
const PostureShareButton: React.FC<CombinedPostureShareButtonProps> = (
  props
) => {
  ...
  const asanaContextRaw = useAsanaPosture()
  ...
  const validateContextData = useCallback(
    (content: ShareableContent | null): ContextValidationResult => {
      ...
      if (!asanaContextRaw)
        contextProviderStatus.missingProviders.push('AsanaPostureProvider')
      if (!seriesContextRaw)
        ...
        switch (content.contentType) {
          ...
          case 'series': {
            ...
            if (contextSeries && contextSeries.seriesName) {
              ...
              // Check for missing critical data
              if (
                !seriesData.seriesPostures?.length &&
                !contextSeries.seriesPostures?.length
              ) {
                result.errors.push(
                  'Series has no postures in both props and context'
                )
                result.isValid = false
              }
            }
  ...
  const validateContentData = (content: ShareableContent | null): boolean => {
    ...
    switch (content.contentType) {
      ...
      case 'series': {
        const seriesData = content.data as FlowSeriesData
        return !!(seriesData.seriesName && seriesData.seriesPostures?.length)
      }
  ...
  const content: ShareableContent | null = React.useMemo(() => {
    ...
    try {
      // Try AsanaPostureContext first (for asana content type or general fallback)
      if (asanaContext?.state?.postures) {
        const contextAsana = asanaContext.state.postures
        if (
          contextAsana &&
          contextAsana.id &&
  ...
  React.useEffect(() => {
    ...
    if (sessionCompatibility.warnings.length > 0) {
      console.warn(
        'PostureShareButton session compatibility warnings:',
    ...
    if (!contextValidation.contextProviderStatus.dependencyChainValid) {
      console.warn(
        'PostureShareButton provider chain incomplete:',
    ...
    if (sessionCompatibility.sessionState === 'loading') {
      console.warn(
        'PostureShareButton session state loading - context data may be incomplete'
      )
    }
  }, [
    contextValidation,
