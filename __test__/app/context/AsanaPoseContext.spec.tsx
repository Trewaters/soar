/**
 * Unit tests for AsanaPoseContext with multi-image support
...
import AsanaPoseProvider, {
  useAsanaPose,
} from '../../../app/context/AsanaPoseContext'
...
const TestWrapper = ({ children }: { children: ReactNode }) => (
  <SessionProvider session={mockSession}>
    <ThemeProvider theme={theme}>
      <CssBaseline />
      <AsanaPoseProvider>{children}</AsanaPoseProvider>
    </ThemeProvider>
  </SessionProvider>
)
...
describe('AsanaPoseContext', () => {
  ...
  describe('Context Provider', () => {
    it('should provide initial state with proper defaults', () => {
      const { result } = renderHook(() => useAsanaPose(), {
        wrapper: TestWrapper,
      })
      ...
      expect(result.current.state.poses.imageCount).toBe(0)
      expect(result.current.state.poses.poseImages).toEqual([])
    })
    ...
    it('should throw error when used outside provider', () => {
      ...
      expect(() => {
        renderHook(() => useAsanaPose())
      }).toThrow('useAsanaPose must be used within an AsanaPoseProvider')
  ...
  describe('State Management', () => {
    it('should set poses and reset carousel index', () => {
      const { result } = renderHook(() => useAsanaPose(), {
        wrapper: TestWrapper,
      })
      ...
      expect(result.current.state.poses).toEqual(mockAsana)
    ...
    it('should update image count', () => {
      ...
      expect(result.current.state.poses.imageCount).toBe(3)
    })
  ...
  describe('Image Management', () => {
    it('should add pose image and update count', () => {
      ...
      expect(result.current.state.poses.poseImages).toContain(mockPoseImage)
      expect(result.current.state.poses.imageCount).toBe(1)
    })
    ...
    it('should add multiple pose images', () => {
      ...
      expect(result.current.state.poses.poseImages).toHaveLength(2)
      expect(result.current.state.poses.imageCount).toBe(2)
    })
    ...
    it('should remove pose image and update count', () => {
      ...
      expect(result.current.state.poses.poseImages).toHaveLength(0)
      expect(result.current.state.poses.imageCount).toBe(0)
    })
    ...
    it('should reorder images correctly', () => {
      ...
      expect(result.current.state.poses.poseImages).toEqual(reorderedImages)
    })
  })

  describe('Integration with Initial State', () => {
    it('should handle pose with existing images', () => {
      const { result } = renderHook(() => useAsanaPose(), {
        wrapper: TestWrapper,
      })
      ...
      expect(result.current.state.poses.imageCount).toBe(2)
      expect(result.current.state.poses.poseImages).toHaveLength(2)
      expect(result.current.state.currentImageIndex).toBe(0) // Reset on pose change
    })
    ...
    it('should maintain backward compatibility with single image asanas', () => {
      ...
      expect(result.current.state.poses.imageCount).toBe(1)
      expect(Array.isArray(result.current.state.poses.poseImages)).toBe(true)
      expect(result.current.state.poses.poseImages).toHaveLength(0)
    })
  })
