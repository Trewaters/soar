...
import Page from '@app/sequences/practiceSequences/page'
...
jest.mock('next/navigation', () => ({
  ...
  useRouter: jest.fn(),
  usePathname: jest.fn(() => '/sequences/practiceSequences'),
}))
...
type SequenceDataTestShape = SequenceData & {
  createdBy?: string
}

const createMockSequence = (
  overrides: Partial<SequenceDataTestShape> = {}
): SequenceDataTestShape => ({
  id: 1,
...
describe('Practice Sequences Page - View Toggle Features', () => {
  ...
  describe('View Toggle Icons Display', () => {
    it('should display view toggle icons when a sequence is selected', async () => {
      ...
      // Scroll view icon should be visible (current view) and disabled
  ...
  describe('List View Navigation', () => {
    it('should switch to list view when list view icon is clicked', async () => {
      ...
      // Initially list view button is enabled (scroll view is active)
      expect(listViewButton).not.toBeDisabled()

      await user.click(listViewButton)

      // After clicking, list view button should be disabled (list view is now active)
      await waitFor(() => {
        expect(
      ...
      // Scroll view button should now be enabled
      expect(
        screen.getByRole('button', {
          name: /switch to scroll view for Morning Flow/i,
        })
    ...
    it('should switch to list view with different sequence IDs', async () => {
      const user = userEvent.setup()
      ...
      for (const testSeq of testSequences) {
        ...
        await waitFor(() => {
          expect(
            screen.getByRole('button', {
              name: new RegExp(`switch to list view for ${testSeq.name}`, 'i'),
            })
          ).toBeDisabled()
        })
    ...
    it('should not navigate when scroll view icon is clicked (current view with pointer-events: none)', async () => {
      ...
      // Should be present and disabled (current view)
      expect(scrollViewButton).toBeInTheDocument()
      expect(scrollViewButton).toBeDisabled()
  ...
  describe('Edit Navigation', () => {
    it('should navigate to edit mode when edit icon is clicked', async () => {
      ...
      expect(mockReplace).toHaveBeenCalledWith(
        '/sequences/practiceSequences?id=456&edit=true'
      )
    })
    ...
    it('should include edit parameter in URL for edit navigation', async () => {
      ...
      // Select the edit button - use more specific selector to avoid matching multiple buttons
      const editButton = screen.getByRole('button', {
        name: /Edit Morning Flow/i,
      })
      ...
      expect(callArgs).toBe('/sequences/practiceSequences?id=789&edit=true')
    })
  })
  ...
  describe('View Toggle Icon States', () => {
    it('should show scroll view icon as current view with orange color', async () => {
      ...
      // Should be disabled (current view)
      expect(scrollViewButton).toBeInTheDocument()
      expect(scrollViewButton).toBeDisabled()
    })
  ...
  describe('Accessibility for View Toggle', () => {
    ...
    it('should indicate disabled state in ARIA label for current view', async () => {
      ...
      // Current view icon is disabled
      expect(scrollViewButton).toBeDisabled()
      expect(scrollViewButton).toHaveAttribute('aria-pressed', 'true')
    })
  })
